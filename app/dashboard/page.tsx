...
import { ArrowLeft, Check, ImagePlus, LayoutDashboard, LoaderCircle, Plus, ShoppingBag, Trash2, X } from 'lucide-react';
...
export default function MemberDashboard() {
  ...
  return <main className="min-h-screen bg-[#f4f1eb] text-foreground">
    ...
    <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 md:px-10 lg:grid-cols-[1.2fr_0.8fr]">
      <section><div className="flex items-end justify-between border-b border-border pb-4"><div><p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Private to your account</p><h2 className="font-heading text-3xl">My submissions</h2></div><span className="text-sm text-muted-foreground">{myWorks.length} works</span></div>{myWorks.length ? <div className="mt-5 grid gap-4 sm:grid-cols-2">{myWorks.map((work) => <article key={work.id} className="overflow-hidden border border-border bg-background"><div className="aspect-[4/3] bg-muted">{work.image && <img src={work.image} alt={work.title} className="h-full w-full object-cover" />}</div><div className="p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-heading text-xl">{work.title}</h3><p className="text-xs text-muted-foreground">{work.artist} · {work.year}</p></div><Badge variant="outline" className="rounded-full capitalize">{work.submissionStatus}</Badge></div><p className="mt-3 text-xs text-muted-foreground">{work.submissionStatus === 'pending' ? 'Waiting for gallery approval. Only you and the owner can see this work.' : work.published ? 'Published in the collection.' : 'Not currently published.'}</p><Button variant="ghost" className="mt-3 w-full rounded-none text-red-700" onClick={() => deleteSubmission(work)}><Trash2 /> Delete submission</Button></div></article>)}</div> : <div className="mt-5 grid min-h-56 place-items-center border border-dashed border-border bg-background text-center"><div><ImagePlus className="mx-auto mb-3 size-6" /><p className="font-heading text-2xl">No submissions yet</p><p className="mt-1 text-sm text-muted-foreground">Upload a work for private review.</p></div></div>}</section>
