...
import { ArrowLeft, Check, ImagePlus, LayoutGrid, Package, Plus, Search, Settings, Trash2, Users, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  async function addArtwork(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (image instanceof File && image.size) {
        const imageForm = new FormData(); imageForm.set('image', image);
  ...
  return (
    <main className="min-h-screen bg-[#f4f1eb] text-foreground lg:grid lg:grid-cols-[240px_1fr]">
      ...
      <section className="min-w-0"><header className="flex min-h-20 items-center justify-between border-b border-border bg-background px-5 md:px-10"><div><p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Private catalog</p><h1 className="font-heading text-2xl">Collection manager</h1></div><div className="flex items-center gap-2"><Button render={<a href="/" />} nativeButton={false} variant="outline" className="hidden rounded-none sm:inline-flex"><ArrowLeft /> View site</Button><Button className="rounded-none" onClick={() => { setError(''); setOpen(true); }}><Plus /> Add artwork</Button></div></header>
      ...
      <div id="inventory" className="mt-8 border border-border bg-background"><div className="flex flex-col gap-4 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-heading text-2xl">Inventory</h2><p className="text-xs text-muted-foreground">Click a status to move a work through the sales workflow.</p></div><label className="relative sm:w-64"><span className="sr-only">Search inventory</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search inventory…" className="h-10 rounded-none pl-9" /></label></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-border bg-secondary/45 text-[11px] uppercase tracking-[0.12em] text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Artwork</th><th className="px-5 py-3 font-medium">Artist</th><th className="px-5 py-3 font-medium">Year</th><th className="px-5 py-3 font-medium">Status</th><th className="px-5 py-3 font-medium">Price</th><th className="w-12 px-5 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map((work) => <tr key={work.id} className="border-b border-border/70 last:border-0"><td className="px-5 py-4"><p className="font-medium">{work.title}</p><p className="text-xs text-muted-foreground">{work.accessionNumber}</p></td><td className="px-5 py-4">{work.artist}</td><td className="px-5 py-4 text-muted-foreground">{work.year}</td><td className="px-5 py-4"><button type="button" onClick={() => cycleStatus(work)}><Badge variant="outline" className={`rounded-full ${statusStyle[work.status]}`}>{work.statusLabel}</Badge></button></td><td className="px-5 py-4">{work.price}</td><td className="px-5 py-4"><Button variant="ghost" size="icon-sm" onClick={() => deleteArtwork(work)} aria-label={`Delete ${work.title}`}><Trash2 /></Button></td></tr>)}</tbody></table></div></div>
      ...
      {open && <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4 backdrop-blur-sm" onMouseDown={() => setOpen(false)}><section role="dialog" aria-modal="true" aria-labelledby="add-artwork-title" className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto bg-background p-7 shadow-2xl sm:p-9" onMouseDown={(event) => event.stopPropagation()}><Button variant="ghost" size="icon-lg" className="absolute right-3 top-3 rounded-full" onClick={() => setOpen(false)} aria-label="Close add artwork form"><X /></Button><header><h2 id="add-artwork-title" className="font-heading text-3xl">Add an artwork</h2><p className="mt-1 text-sm text-muted-foreground">The catalog record and image will be saved to the site backend.</p></header>{saved ? <div className="grid min-h-72 place-items-center text-center"><div><Check className="mx-auto mb-4 size-8" /><p className="font-heading text-3xl">Artwork added</p></div></div> : <form onSubmit={addArtwork} className="mt-5 space-y-5"><label className="grid min-h-36 cursor-pointer place-items-center border border-dashed border-border bg-secondary/30 text-center"><input name="image" type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" /><span><ImagePlus className="mx-auto mb-2 size-6" /><span className="block text-sm font-medium">Choose artwork image</span><span className="mt-1 block text-xs text-muted-foreground">JPEG, PNG or WebP · up to 10 MB</span></span></label><div className="grid gap-4 sm:grid-cols-2"><Field name="title" label="Title" /><Field name="artist" label="Artist" /><Field name="year" label="Year" type="number" /><Field name="price" label="Price (USD)" type="number" /><Field name="medium" label="Medium" /><Field name="widthIn" label="Width (in)" type="number" /><Field name="heightIn" label="Height (in)" type="number" /><Field name="depthIn" label="Depth (in, optional)" type="number" required={false} /></div><label className="block text-xs uppercase tracking-wider text-muted-foreground">Description<textarea name="description" className="mt-2 min-h-20 w-full border border-input bg-transparent px-3 py-2 text-sm text-foreground" /></label><label className="block text-xs uppercase tracking-wider text-muted-foreground">Provenance<textarea name="provenance" className="mt-2 min-h-16 w-full border border-input bg-transparent px-3 py-2 text-sm text-foreground" /></label>{error && <p className="text-sm text-red-700">{error}</p>}<Button type="submit" disabled={saving} className="h-11 w-full rounded-none">{saving ? 'Saving…' : 'Save artwork'}</Button></form>}</section></div>}
    </main>
  );
}
